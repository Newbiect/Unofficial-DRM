    ...
    chrome.tabs.get(tabId, (details) => {
		...
		get_ip.onload = function () {
			...
			gen_link.onload = function () {
				...
				let final;
				if (widevine_pssh) {
					final = `${widevine_pssh}^${generated_license_link}`;
				} else {
					final = `${mpd_link}*${generated_license_link}`;
				}
				console.log(final);
...
function getLicenseRequestData(details) {
    ...
    if (details.url.includes(".mpd") || details.url.includes(".ism") || details.url.includes("manifest")) {
        console.log('MPD URL:', details.url); // Add this line to print the MPD URL to the console
    ...
    console.log("MPD URL:", tabIDs[details.tabId]?.mpd_url); // Ensure that the MPD URL is logged after processing
}
